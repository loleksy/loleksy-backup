...
export class VideoMediaOptimizer implements MediaOptimizerInterface {
  ...
  constructor(sourcePath: string, progressCallback: (uploaded: number) => void) {
  ...
  public async optimize(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      hbjs.spawn({ 
        ...
        encoder: 'x264',
        // video
        encopts: 'x264 Unparse: level=3.1:ref=5:direct=auto:subme=8:trellis=2:vbv-bufsize=17500:vbv-maxrate=17500:rc-lookahead=50',
        vb: 800,
        "two-pass": true,
        "no-turbo": true,
        // picture
